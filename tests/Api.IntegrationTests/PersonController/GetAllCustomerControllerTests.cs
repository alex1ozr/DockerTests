...
public class GetAllCustomerControllerTests : IAsyncLifetime
{
    ...
    public GetAllCustomerControllerTests(PersonApiFactory apiFactory)
    {
    ...
    public async Task GetAll_ReturnsAllCustomers_WhenCustomersExist()
    {
        ...
        var customersResponse = await response.Content.ReadFromJsonAsync<IReadOnlyCollection<PersonResponse>>();
        customersResponse!.Single().Should().BeEquivalentTo(createdCustomer);
    }
    ...
    public async Task GetAll_ReturnsEmptyResult_WhenNoCustomersExist()
    {
        ...
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var customersResponse = await response.Content.ReadFromJsonAsync<IReadOnlyCollection<PersonResponse>>();
        customersResponse!.Should().BeEmpty();
    }
